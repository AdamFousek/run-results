      ...
      <form @submit.prevent="pairRunner" v-else class="mt-6">
        <template v-if="form.runnerId === 0">
          ...
          <div v-else class="relative">
            <ul v-if="state.runners.length" class="absolute top-0 w-full z-10 bg-white max-h-32 list-group overflow-y-scroll shadow-md">
              <li v-for="runner in state.runners" :key="runner.id" class="list-group-item p-2 my-1 hover:cursor-pointer hover:bg-violet-50" @click="selectRunner(runner)">
        ...
        <template v-else>
          <div class="mt-4 flex justify-between gap-2 items-center">
            ...
            <div class="flex-1">
              <span class="text-xl">{{ selectedRunner.last_name }} {{ selectedRunner.first_name }}</span> <span class="text-sm text-gray-500">{{ selectedRunner.year }}</span>
            </div>
          </div>
          ...
          <div class="mt-4 flex items-center gap-4">
            <NButton :loading="form.processing" icon-placement="left" :disabled="form.processing" attr-type="submit" round type="primary">{{ $t('runnerPair.pair') }}</NButton>
